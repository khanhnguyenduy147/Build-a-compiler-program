...
{
	int ln = lineNo;
	while ((currentChar != EOF) && (ln == lineNo))
	{
		readChar();
	}
}
...
Token *getToken(void)
{
	...
	case CHAR_PLUS:
		...
		{
			readChar();
			return makeToken(SB_AS_ADD, ln, cn);
		}
		else
	...
	case CHAR_TIMES:
		...
		{
			readChar();
			return makeToken(SB_AS_MUL, ln, cn);
		}
		else
	...
	case CHAR_PERIOD:
		...
		{
			readChar();
			return makeToken(SB_RBRACK, ln, cn);
		}
		else
	...
	case CHAR_LPAR:
		...
		case CHAR_PERIOD:
			readChar();
			return makeToken(SB_LBRACK, ln, cn);
		case CHAR_TIMES:
			readChar();
			skipComment();
			return getToken();
	...
	case CHAR_RPAR:
		...
		return token;
	case CHAR_LBRACK:
		token = makeToken(SB_LBRACK, lineNo, colNo);
		readChar();
		return token;
	case CHAR_RBRACK:
		token = makeToken(SB_RBRACK, lineNo, colNo);
		readChar();
		return token;
	case CHAR_QUESTION:
