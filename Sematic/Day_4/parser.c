...
void compileForSt(void) {
  ...
  Object *variable = checkDeclaredVariable(currentToken->string);
  checkBasicType(variable);
  eat(SB_ASSIGN);
  ...
  //Check Equality of 3 type
  checkTypeEquality(variable->varAttrs->type, type1);
  checkTypeEquality(type1, type2);
...
void compileArgument(Object* param) {
  ...
  Type *actuaType = compileExpression();
  checkTypeEquality(actuaType, param->paramAttrs->type);
}
...
Type* compileIndexes(Type* arrayType) {
  ...
  while (lookAhead->tokenType == SB_LSEL) {
    ...
    Type *intType = compileExpression();
