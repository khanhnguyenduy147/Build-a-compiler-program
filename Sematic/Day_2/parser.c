...
ConstantValue* compileUnsignedConstant(void) {
  ...
  case TK_IDENT:
    eat(TK_IDENT);
    Object *unConstObj = lookupObject(currentToken->string);
    if(unConstObj == NULL){
      error(ERR_UNDECLARED_CONSTANT, currentToken->lineNo, currentToken->colNo);
    } else if(unConstObj->kind != OBJ_CONSTANT){
      error(ERR_INVALID_CONSTANT, currentToken->lineNo, currentToken->colNo);
    } else {
      constValue = (ConstantValue*) malloc(sizeof(ConstantValue));
      *constValue = *(unConstObj->constAttrs->value);
    }
    //FIX
    break;
  case TK_CHAR:
...
ConstantValue* compileConstant2(void) {
  ...
  case TK_IDENT:
    eat(TK_IDENT);
    Object *const2Obj = lookupObject(currentToken->string);
    if(const2Obj == NULL){
      error(ERR_UNDECLARED_CONSTANT, currentToken->lineNo, currentToken->colNo);
    } else if(const2Obj->kind != OBJ_CONSTANT){
      error(ERR_INVALID_CONSTANT, currentToken->lineNo, currentToken->colNo);
    }
    constValue = duplicateConstantValue(const2Obj->constAttrs->value);
    break;
  default:
    error(ERR_INVALID_CONSTANT, lookAhead->lineNo, lookAhead->colNo);
    break;
...
Type* compileType(void) {
  ...
  case TK_IDENT:
    ...
    } else if(typeObj->kind != OBJ_TYPE){
      error(ERR_INVALID_TYPE, currentToken->lineNo, currentToken->colNo);
    }
    type = duplicateConstantValue(typeObj->typeAttrs->actualType);
    break;
  default:
