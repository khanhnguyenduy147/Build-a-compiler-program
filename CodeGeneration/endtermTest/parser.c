...
{
	Object *varObj[MAX]; // Mảng khai báo nhiều biến cùng lúc
	Type *varType;
	...
	if (lookAhead->tokenType == KW_VAR)
	{
		...
		do
		{
			...
			varObj[numVarObjects] = createVariableObject(currentToken->string);
			numVarObjects++;
			...
			while (lookAhead->tokenType == SB_COMMA)
			{
				...
				{
					if (strcmp(currentToken->string, varObj[i]->name) == 0)
					{
...
Type *compileLValue(void)
{
	...
	case OBJ_VARIABLE:
		...
		if (lookAhead->tokenType == SB_AS_ADD || lookAhead->tokenType == SB_AS_MUL)
...
void compileAssignSt(void)
{
	...
	case SB_ASSIGN:
	{
			...
			lookAhead->tokenType == SB_GT)
		{
			...
			expTypeExtra = compileExpression();
			checkTypeEquality(expType, expTypeExtra);
			...
			eat(SB_QUESTION);
			assignVal1 = compileExpression();

			eat(SB_COLON);
			jInstruction = genJ(DC_VALUE);
			updateFJ(fjInstruction, getCurrentCodeAddress());

			assignVal2 = compileExpression();
			checkTypeEquality(varType, assignVal1);
			checkTypeEquality(varType, assignVal2);
	...
	case TK_IDENT:
		...
		case OBJ_FUNCTION:
			...
			else
			{
				genINT(RESERVED_WORDS);
				compileArguments(obj->funcAttrs->paramList);
				genDCT(RESERVED_WORDS + obj->funcAttrs->paramCount);
				genFunctionCall(obj);
			}
...
Type *compileIndexes(Type *arrayType)
{
	...
	while (lookAhead->tokenType == SB_LBRACK)
	{
		eat(SB_LBRACK);
		type = compileExpression();
		checkIntType(type);
